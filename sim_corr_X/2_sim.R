  ...
  if (ep == "continuous") {
    tryCatch({
      ...
      res_alt <- list(
        ...
        ec_borrow(Y, A, S, X, "AdaLasso Selective Borrow ACW", fam, n_fisher = NULL),
        ec_borrow(Y, A, S, X, "Conformal Selective Borrow AIPW", fam, n_f,
                  gamma_sel = 0.4, output_frt = o_f) %>%
          intFRT:::add_name("(g=0.4)"),
        ec_borrow(Y, A, S, X, "Conformal Selective Borrow AIPW", fam, n_f,
                  gamma_sel = 0.6, output_frt = o_f) %>%
          intFRT:::add_name("(g=0.6)"),
        ec_borrow(Y, A, S, X, "Conformal Selective Borrow AIPW", fam, n_f,
                  gamma_sel = 0.8, output_frt = o_f) %>%
          intFRT:::add_name("(g=0.8)"),
        ec_borrow(Y, A, S, X, "Conformal Selective Borrow AIPW", fam, n_f, 
      ...
      res_null <- list(
        ...
        ec_borrow(Ynull, A, S, X, "AdaLasso Selective Borrow ACW", fam, n_fisher = NULL),
        ec_borrow(Ynull, A, S, X, "Conformal Selective Borrow AIPW", fam, n_f,
                  gamma_sel = 0.4, output_frt = o_f) %>%
          intFRT:::add_name("(g=0.4)"),
        ec_borrow(Ynull, A, S, X, "Conformal Selective Borrow AIPW", fam, n_f,
                  gamma_sel = 0.6, output_frt = o_f) %>%
